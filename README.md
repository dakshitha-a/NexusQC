...
- **Runs real jobs, not fabricated numbers.** Single-point energies, geometry optimization, vibrational frequencies, CASSCF, CASPT2, TD-DFT/TDA/CIS/TD-HF, EOM-CCSD, molecular orbital visualization, potential energy scans, and NEB-TS transition-state searches — routed automatically to whichever of PySCF, ORCA, or BAGEL is right for the method (and reports plainly if none of them can do what you asked).
- **Nudged Elastic Band transition-state searches, on the ground state or an excited state.** Give the agent a reactant and product structure and it runs ORCA's native NEB-TS path search between them — always asking first whether to pre-optimize the two endpoints, since that has no sensible default. Once complete, the job's detail view shows a frame-by-frame slider over the converged path (with the refined transition-state structure as its own frame), a reaction-path energy plot, and per-frame molecular orbitals rendered from that image's own wavefunction.
- **Not limited to the pre-built job types — write and run a raw ORCA/BAGEL input directly.** For a calculation without its own job type here (an IRC path search, a relaxed surface scan, anything else this app has no dedicated parser for), ask the agent to run it and it composes the complete literal input file itself and submits it through the exact same approval-card + background-execution pipeline as any other job. There's no structured result parsing afterward (no parser exists to do it with) — the job's detail view shows just the input geometry and raw output, plus a download button for the full job files, same as any other job.
- **Can also write (but not run) an input for other QM software.** Ask for a Gaussian/NWChem/Psi4/etc. input and the agent composes the text directly in its reply — grounded in a manual you've uploaded for it, if any — but there's no approval card and no job, since this app has no way to execute anything outside PySCF/ORCA/BAGEL.
- **Potential energy scans run as real parallel jobs, not one slow sequential loop.** Give the agent a start and end geometry (name/SMILES/pasted XYZ for each) and it builds an interpolated path — IDPP by default (aligns the structures, then iteratively avoids atom clashes across the whole path), or true internal-coordinate LIIC, or plain Cartesian interpolation if you ask for it — then spawns one real sub-job per image (any job type, including TD-DFT/CASSCF/CASPT2/EOM-CCSD for a curve per electronic state, not just the ground state) and runs them concurrently under the same resource-aware job manager as everything else. The scan shows up as one job with a live frame slider over the whole path, each image's own sub-job separately viewable (same molecule/output/log tools as any other job) nested underneath, and a downloadable PES plot once it's done — even a partially-failed scan still reports what succeeded rather than losing the whole run. Single-molecule bond/angle/dihedral scans use the same parallel machinery.
...
- **Job manager, not just a status line.** Every job you've submitted shows up in a live table (status, engine, description), with a kill button for anything still running and a detail drawer for full parameters, results, and artifacts once it finishes — including a download button (a ZIP of the job's files for ORCA/BAGEL, a generated text summary for PySCF, which has no literal input/output file), a geometry flyout (the optimized structure for a geometry optimization, the input structure otherwise), and a raw-output flyout for ORCA/BAGEL jobs with a real find bar (Ctrl+F, match count, next/prev, highlighting) for digging through long output.
- **Conversations title themselves — and you can pin or rename them.** A fresh conversation auto-renames from your first message once the agent responds, so you're never left scrolling a sidebar full of "New conversation". Pin the ones you'll come back to so they stay at the top regardless of recency, and rename any conversation with the pencil icon (or just double-click its title).
...
- **Plots UV/Vis and IR spectra from completed jobs**, and says clearly when a job has no oscillator strengths/IR intensities to plot rather than faking one (IR intensities are ORCA/BAGEL only — PySCF's frequency job type doesn't compute them in this app).
