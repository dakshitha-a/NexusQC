...
**Secondary — SSE `agent_step` events, pre-interrupt only.** `_run_turn` publishes `agent_step` exclusively from its streaming `"updates"` loop. `approve_job` resumes via `resume_turn()` — a single blocking `.invoke()` — then calls `_publish_new_messages()`, which emits only `message` events. **Every tool call in the post-resume tail of a turn is invisible to `agent_step`**, including the re-executed `submit_job` itself. Since every job scenario goes through an approval, making this the primary channel would blind the suite to exactly the calls that matter most. Registered as expected-negative `XN-14`; confirmed empirically by `e2e_04_harness_gate.py`.
