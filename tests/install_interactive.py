...
def main() -> None:
    ...
    second_pass(target, log)

    print(f"\ntranscript kept at: {log}")
    summary()


def second_pass(target: Path, log: Path) -> None:
    """Re-run the installer over the deployment the first pass just made.

    Two things can only be reached this way. The override file has to already
    exist and differ before the installer can offer to keep it, and the
    interrupt path needs a long-running step to interrupt.
    """
    ov = target / "docker-compose.override.yml"
    mark = "# HAND-EDITED: a bind mount nothing here could reconstruct if it were lost"
    before = ov.read_text() + f"\n{mark}\n"
    ov.write_text(before)

    child = pexpect.spawn(
        "bash", ["scripts/install.sh"], cwd=str(target), encoding="utf-8", timeout=600,
        env=dict(os.environ, TERM="dumb"),
    )
    seen: list[str] = []
    child.logfile_read = type("T", (), {"write": seen.append, "flush": lambda s: None})()

    child.expect(r"Continue installing here\?"); child.sendline("y")
    child.expect(r"Reconfigure from scratch\?"); child.sendline("y")
    child.expect(r"Choose 1-\d"); child.sendline("1")
    child.expect(r"Press enter to accept"); child.sendline("")
    i = child.expect([r"Found orca at .* -- use it\?", r"Enter a orca path manually\?"])
    child.sendline("y" if i == 0 else "n")
    i = child.expect([r"Found BAGEL at .* -- use it\?", r"Enter a BAGEL path manually\?"])
    child.sendline("y" if i == 0 else "n")
    i = child.expect([r"Install it\? \[y/N\]", r"enter its path\?"])
    if i == 1:
        child.sendline("n")
        child.expect(r"Install it\? \[y/N\]")
    child.sendline("n")

    child.expect(r"Replace it\?")
    text = "".join(seen)
    check(
        "an existing override that differs is not replaced without asking",
        "not tracked by git, so replacing it is final" in text,
        "warned, then asked",
        text[-1500:],
    )
    shown = text.split("Replace it?")[0][-4000:]
    check(
        "the diff it shows names the hand-edited line",
        mark in shown,
        "the added line is in the diff",
        shown,
    )
    # The generated file carries the time it was written. Comparing on that
    # would make the file differ from itself on every re-run and reduce this
    # confirmation to a date change, which is how people learn to say yes
    # without reading.
    check(
        "the diff is not just a changed timestamp",
        "# Generated by scripts/install.sh on" not in shown,
        "the timestamp line is excluded from the comparison",
        shown,
    )
    child.sendline("n")
    child.expect(r"kept the existing docker-compose.override.yml")
    check(
        "answering no leaves the file byte for byte as it was",
        ov.read_text() == before,
        "unchanged on disk",
        f"changed to: {ov.read_text()!r}",
    )

    # Ctrl-C during the build. Sent to the process group, as a terminal would,
    # so the build dies too rather than the trap waiting on it.
    child.expect(r"building the images", timeout=300)
    ...
    check(
        "an interrupt says which step it was in, instead of dying mutely",
        "cancelled during: building the images" in text,
        "named the step",
        text[-1500:],
    )
    check(
        "and says a re-run is safe",
        "safe to run again" in text,
        "said so",
        text[-1500:],
    )
