...
_PYSCF: tuple[MethodCaps, ...] = (
    ...
    MethodCaps(
        ...
        notes="Multi-configuration pair-density functional theory, from pyscf-forge. The "
              ...
              "separately, so states can come out reordered against their MCSCF labels. When "
              "that happens the result says so rather than leaving it to be noticed: the "
              "summary carries a state_order_note, the reported total energy is the lowest "
              "state rather than the first-labelled one, and the excitation energies are "
              "measured from the lowest state, so none of them can come out negative. No "
