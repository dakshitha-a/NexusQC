...
- **Long CASSCF/CASPT2 runtimes are expected, not a defect** (F-017, corrected 2026-08-17). Runs of 40–50 minutes, and sometimes hours, are routine in this group, and the asynchronous job system exists precisely so that a user can submit, leave, and come back. Do **not** warn users away from these combinations or add a "this will be slow" gate. The narrow factual point that remains: BAGEL's macro-iterations on *this host* are ~85s where ORCA and PySCF are sub-second for the same trivial system — an outlier attributable to this host's MKL/BAGEL install — so ORCA or PySCF is the better choice when someone wants a fast turnaround on a small system. BAGEL `frequency`, `mo_visualization` and `custom` are unaffected (26s, 22s, 87s).
- **The leave-and-return workflow is the load-bearing property for long jobs, and it is now tested.** `tests/e2e/e2e_17_logout_and_return.py` confirms end-to-end (21/21) that logging out never kills a running calculation, that it completes with no session open, and that the job, its results, its artifacts and the conversation are all waiting on return — including a results summary the agent wrote unprompted while nobody was logged in.
