...
All six steps done, verified in-process and against the live stack.

The shape of the bug is worth remembering, because it was fixed once already
and came back one level out. The gap between admitting a job and that job's
status reaching disk is real and cannot be closed; anything that counts
running jobs has to account for what is in that gap. The first fix counted it
with two variables local to one dispatch pass, which is correct only if a
pass is the longest anything stays in the gap, and a burst submission makes
passes fire far faster than the pool writes. The set now spans admission to
release, which is the actual width of the gap, and is unioned rather than
added so it stays correct however it overlaps what the disk already shows.

What this changed:

- `app/chemistry/jobs/scheduler.py`: `_in_flight`, `release()`, and a
  `block_reason(job_id, in_flight)` contract in place of two counters.
- `app/chemistry/jobs/base.py`: the union in
  `_concurrent_jobs_block_reason`, and the release on every path out of an
  admitted job.
- `tests/backend/perf_05_admission_cap_arithmetic.py`: the cross-tick cases,
  and a harness that models the release as well as the disk write.
- `tests/backend/perf_06_admission_release_wiring.py`: new, covering the
  release half, where a missed call leaks a cap slot permanently.
