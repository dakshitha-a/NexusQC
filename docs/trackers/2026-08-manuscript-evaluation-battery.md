# Closed Tracker: manuscript evaluation battery

**Closed 2026-08-25 with the battery run and the results recorded**, but not
with every row green: condition C's C-07 is still a `harness-error` rather than
a verdict, and the trials that failed for reasons now parked in `BACKLOG.md`
are to be re-run once those fixes land. That re-run is the closing phase of the
tracker that replaced this one, not unfinished work here.

Results live in `rsc_digital_discovery/evaluation/summary.md` (the paper's
conditions table, the tier R deviations, every failure and every recomputed
verdict) and `results/` (one score sheet per trial). Headline: 297 trials,
grounding 10/10 tasks, phrasing robustness 15/15, cross-job chains 8/8,
reproduction 6/6 with a largest deviation of 0.05 cm-1, and a model sweep in
which the 8B collapses outright and the 14B degrades on tasks and elicitation
while holding grounding at 12/12.
