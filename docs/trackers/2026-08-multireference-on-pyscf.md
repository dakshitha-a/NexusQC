...
**Archived on 2026-08-30** when the next plan started. The active tracker is
always [`../TRACKER.md`](../TRACKER.md), and there is exactly one at a time.
...
once. Each plan, feature or non-trivial request gets its own tracker, the file
at `docs/TRACKER.md` is whichever one is currently in motion, and when its plan
is finished the file is closed out and moved here, then a fresh one starts for
whatever comes next.
...
replaces is
[`2026-08-clearing-the-backlog-round-2.md`](2026-08-clearing-the-backlog-round-2.md)
