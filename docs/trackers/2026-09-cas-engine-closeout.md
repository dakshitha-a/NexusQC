...
**Closed 2026-09-05.** Fifteen phases, 43 steps.
...
- [done] P14.1: Every surface that describes the engine
  evidence: README.md -> "the capability table now says the recommendation is validated for organic molecules and points at the method document; the basis parameter's warning says a Rydberg state is identified and then deliberately left out; the job drawer shows the reference energy beside the excitation energies and the states-not-looked-for list; CHANGELOG has the entry. All 17 cas_* scripts pass"
