...
| `QC_AGENT_BUILD_COMMIT` | `unknown` | The commit the api image was built from, set by `scripts/install.sh` and `scripts/update.sh` as a build argument and read back by `/api/version` and by `update.sh` to know what is deployed. Not something to set in `.env`; a hand-run build leaves it `unknown`, which `update.sh` reads as "cannot tell" |
| `QC_AGENT_PUBLIC_URL` | *unset* | The origin other people use to reach the deployment (`https://<name>:<port>`), the base of every invite and password-reset link. Set by the installer; overridable in the admin console's Deployment section. Unset means the admin's own browser address, which fails on a Tailscale node shared with each user. See [DEPLOYMENT.md](DEPLOYMENT.md#how-this-deployment-is-reached) |
| `QC_AGENT_BUILD_VERSION` | `unknown` | The same commit as `git describe` names it: a release tag, a tag plus a distance, or a short sha before the first release. Shown under Help → About and quoted in bug reports. Stamped alongside the commit by the same scripts |
...
| `single_point/ee` | `want_oscillator_strengths` | `False` | For CASSCF this routes to BAGEL, the preferred engine here for intensities. ORCA computes them too and is used if named; PySCF cannot |
