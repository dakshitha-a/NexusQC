...
- [todo] P1.1: A failing test that admits more than the cap allows
- [todo] P1.2: The same test shows the round-robin order is a consequence, not the cause

## Phase 2: The cap counts what has been admitted, not only what is on disk

- [todo] P2.1: Admitted-but-not-yet-running jobs are tracked across ticks
- [todo] P2.2: They are released on the terminal transition that already wakes the dispatcher
- [todo] P2.3: Nothing leaks the set when a job is cancelled between admission and running
