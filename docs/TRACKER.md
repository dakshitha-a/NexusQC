...
- [done] P1.1: A resource_shares table and its model helpers
  evidence: tests/backend/share_03_lifecycle.py -> "34/34, and the two checks that exercise the schema itself are 'a second PENDING offer of the same job to the same user is refused' (409) alongside 're-offering after a decline is allowed' (200), which together prove resource_shares_pending_idx is PARTIAL on status='pending' rather than a total unique constraint; and 'a settled offer cannot be accepted twice' (409), which is set_share_status's compare-and-set carrying status='pending' in its WHERE, so two accept clicks racing leave one winner rather than copying the job twice. Confirmed structurally with psql \\di resource_shares*: the table plus all three indexes present after the api container rebuilt"
